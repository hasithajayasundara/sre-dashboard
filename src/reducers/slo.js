  ...
  switch (action.type) {
    case FETCH_SLO_FILTERS_SUCCESS:
      sloTypes = action.payload.sloTypes.map(itm => ({
        ...itm,
        checked: false,
        services: itm.services.map(service => ({ ...service, checked: false }))
      }));
      return {
        ...state,
        filters: {
    ...
    case SET_SLO_TYPE:
      ...
      return {
        ...
        filters: {
          ...
          sloTypes: [...sloTypes],
          sloServices: [
            ...sloTypes[action.payload.index].services.map(service => ({
              ...service,
              checked: true
            }))
          ]
    ...
    case SET_SLO_TIME_FORMAT:
      let isRelativeTimeSelected = state.filters.timeRange.relativeTimeSelected;
      let to = 0;
      let from = 0;
